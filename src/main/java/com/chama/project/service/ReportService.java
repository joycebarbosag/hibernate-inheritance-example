...
import com.chama.project.repository.ReportRepository;
...
public class ReportService {
    ...
    public ResponseEntity<ReportModel> updateReport(Long id, ReportModel reportUpdate) {
        try {
    ...
    public ReportModel updateReportStatusOrFire(Long id, String isFire, String ativo) {

        ReportModel report = reportRepository.findById(id).orElse(null);
        
        if (report == null) {
            return null; 
        }

        if ("Y".equalsIgnoreCase(isFire)) {
            report.setFire(true);
        } else if ("N".equalsIgnoreCase(isFire)) {
            report.setFire(false);
        }

        if ("Y".equalsIgnoreCase(ativo)) {
            report.setStatus(true);
